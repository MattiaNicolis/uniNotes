...
\begin{enumerate}[itemsep=\smallskipamount]
    ...
    \end{enumerate}
    \item \textbf{merge}: vengono unite le rune in un'unica run
\end{enumerate}
...
\esempio{algoritmo Z-way-Sort-merge}{
    ...
    \smallskip
    Si hanno 4 pagine composte da 3 tuple ciasuna e si vuole ordinare in base alla prima colonna:
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.6\textwidth]{images/ordinamento-Z-way-Sort-Merge.png}
    \end{figure}

    Duramte il sort interno si carica e ordina una pagina alla volta ottenendo delle run ordinate.

    Si ottengono 4 run ordinate che passano alla fase di merge a due vie ($\mathrm{Z} = 2$) e si ottengono 2 run ordinate che passano ad un'ulteriore di merge, ottenendo così un'unica run ordinata.

    Con $\mathrm{NB} = 3$ si hanno solamente 3 buffere a disposizione, quindi, si associano 2 buffer alla run da passare alla fase di merge e 1 buffer per l'output.
...
\begin{itemize}[label=$\Rightarrow$, itemsep=\smallskipamount]
    ...
    \begin{itemize}[label=$\diamond$, itemsep=\smallskipamount]
        \item fase di sort interno: si leggono $\mathbf{NP(R)}$ pagine e si scrivono $\mathbf{NP(R)}$ pagine
        \[
            \mathbf{2 \cdot NP(R)}
        \]
        \item fase di merge: si leggono $\mathbf{NP(R)}$ pagine e si scrivono $\mathbf{NP(R)}$ pagine:
        \[
            \mathbf{2 \cdot NP(R)}
        \]
    \end{itemize}
...
\subsection{Join}
Il join è l'operazione più costosa da eseguire, quindi, l'ottimizzazione è particolarmente attento ad ottimizzare questa operazione, tanto da prevedere diversi algoritmi:
\begin{itemize}[label=$\triangleright$, itemsep=\smallskipamount]
...
Dal punto di vista logico è commutativo, ma dal punto di vista fisico c'è una differnza sostanziale tra $\mathbf{A \bowtie B}$ e $\mathbf{B \bowtie A}$ che cambia le prestazioni.

\subsubsection{Nested loop join}
Date 2 relazioni in imput R e S tra cui è definito il predicato di join PJ, distinguiamo:
\begin{itemize}[label=$\circ$, itemsep=\smallskipamount]
...
\begin{codiceSQL}
\forall tupla t_R { --relazione esterna
    \forall tupla t_S { --relazione interna
...
\esempio{algoritmo nested loop join}{
    ...
    R devve verificare per ogni riga di S che il predicato sia verificato.
}
...
\begin{itemize}[label=$\Rightarrow$, itemsep=\smallskipamount]
    ...
    \begin{itemize}[label=$\diamond$, itemsep=\smallskipamount]
        ...
        \item per ogni riga di R ($\mathrm{NP(R) = \# RIGHE R}$) bisgona leggere completamente S
    \end{itemize}
...
A seconda del numero di righe di R cambia il numero di volte che bisogna leggere $\mathrm{NP(R)}$, quindi, la relazione esterna deve essere quella più piccola per minimizzare il costo.

\subsubsection{Nested loop join con indice}
Data una relazione esterna R e una interna S, si ha che la scansione completa di S può essere sostituita dalla scansione basata sull'indice costituito sugli attributi di join di S.

\esempio{algoritmo nested loop join con indice}{
    Consieriamo il seguente esempio di nested loop join con indice tra le relazioni R e S:
    \begin{figure}[H]
        \centering
...
La tabella R deve comunque essere caricata tutta ($\mathrm{NP(R)}$) e rimane invariato anche il numero di volte che si esegue il ciclo esterno ($\mathrm{NR(R)}$).\\[0.2ex]
Ciò che cambia è il numero di blocchi/pagine di S che si devono caricare tutte le pagine di ($\left(\text{ProfonditàIndice} + \mathbf{\frac{NR(S)}{VAL(A,S)}}\right)$).\\[0.2ex]
L'idea è, invece, di caricare tutte le pagine di S si caricano solo quelle che contengono $\mathrm{S.A = R.A}$
\begin{itemize}[label=$\diamond$, itemsep=\smallskipamount]
    \item $\mathrm{\frac{NR(S)}{VAL(A,S)}}$: rappresenta la stima di quanti blocchi di S contengono A uguale al valore interessato.
    
    Questa stima è anche selettività dell'attributo A.
    \item $\mathrm{VAL(A,S)}$: nunmero di valori distinti di A che compaiono in S
\end{itemize}

\esempio{}{
    Supponiamo che $A = \text{Nome\_Regioni}$, quindi, il numero di possibili valori di A è 21.
    ...
    Quindi, i costo del nested loop join con indice è:
    \[
        \begin{aligned}
            &\mathbf{NP(R) + NR(R)} \cdot \left(\text{ProfonditàIndice} + \mathbf{\frac{NR(S)}{VAL(A,S)}}\right)\\
...
Il blocj nested loop join è una variante del nested loop in cui ciasuna pagina di S (tabella interna) viene letta per ogni pagina di R (tabella esterna), invece, che per ogni riga di R.\\[0.2ex]
Lo pseudocodice è il seguente:
\begin{codiceSQL}
\forall pagina P_R di R { --relazione esterna
    \forall pagina P_S di S { --relazione interna
        \forall tupla t_R di P_R {
...
Il merge scan join è applicabile quando entrambi gli insiemi di tuple in input sono ordinati sugli attributi di join e il join è un equi-join (predicato di join è una condizone di uguaglianza).\\[0.2ex]
Cià accade sia per entrambe le relazioni R e S vale almeno una delle seguenti condizioni:
\begin{itemize}[label=$\circ$, itemsep=\smallskipamount]
...
L'algoritmo di hash-based join è applicabile solo in caso di equi-join, non richieste nè la presenza di indici nè input ordinati e risulta particolarmente vataggioso in caso di relazioni molto grandi.\\[0.2ex]
L'idea sui casi si basa l'hash join è semplice: si suppone di avere a disposizione una funzione hash h, che viene applictaa agli attributi di join delle due relazioni (es. R.J e S.J).

\begin{itemize}[label=$\circ$, itemsep=\smallskipamount]
    \item se tR e tS sono 2 tuple di R e S, allora è possibile che sia tR.J = tS.J solo se h(tR.J) = h(tS.J)
    \item se, viceversa, h(tR.J) diverso h(tR.J), allora si ha tR.J diverso tS.J
\end{itemize}

Da questa idea si hanno diverse implementazioni, che hanno in comune il fatto che R e S vengono partizionate sulla base dei valori della funzione di hah h, e che la ricerca di matching tuples aviene solo tra partizioni relative allo stesso valore di h.
...
Data un'espressione otimizzata in algebra i passi da eseguire sono:
\begin{enumerate}[itemsep=\smallskipamount]
    \item si generano tutti i possibili paini di esecuzione (alberi) alternativi (o un sottoinsieme) ottneuti considernando le seguenti dimensioni
    \begin{itemize}[label=$\circ$, itemsep=\smallskipamount]
    ...
    \end{itemize}
    \item si valuta con frmule approssimate il costo di ogni alterntiva in termini di accessi alla memoria secondaria richiesti(è una stima)
    \item si sceglie l'albero con il costo stimato minimo
\end{enumerate}

Nella valutazione delle stime di costo si tiene conto del profilo delle relazioni, solitamente memorizzato nel Data Directory e contente per ogni relazione T:
\begin{itemize}[label=$\circ$, itemsep=\smallskipamount]
    \item stima della cardinailtà (\# tuple): CARD(T)
    \item stima della dimensione di una tupla: SIZE(T)
    \item Stima della dimensione di ciascun attributo A: VAL(A,T)
    \item stima del numero di valori distinti per ogni attributo A: MAX(A,T) e MIN(A,T)
\end{itemize}