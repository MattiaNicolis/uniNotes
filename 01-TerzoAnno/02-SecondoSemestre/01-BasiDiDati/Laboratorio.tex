...
\begin{enumerate}[itemsep=\smallskipamount]
  ...
  \begin{itemize}[label=$\diamond$, itemsep=\smallskipamount]
    \item  è una \texttt{SELECT} innestata. 
    
    Il risultato della \texttt{SELECT} interna è lo schema con nome \texttt{nomeRisultato} su cui fare la \texttt{SELECT} corrente
  \end{itemize}

  \item  \texttt{from\_item [NATURAL] join\_type from\_item [ON condition]}
  \begin{itemize}[label=$\diamond$, itemsep=\smallskipamount]
    \item  è la clausola \texttt{JOIN}
    \item mtodo più sofisticato del primo e che permette di selezionare un sottoinsieme del  prodotto cartesiano di due o più tabelle. 
    
    \texttt{join\_type} specifica il tipo di \texttt{join}.
  \end{itemize}
\end{enumerate}
...
dove:
\begin{itemize}[label=$\triangleright$, itemsep=\smallskipamount]
  \item \texttt{condition}: espressione booleana combinando condizioni semplici $\mathrm{C_{i}}$ con gli operatori \texttt{AND}, \texttt{OR}, \texttt{NOT}
  \begin{enumerate}
    \item  $C_{i}$ \texttt{:= expr [ $\theta$ \{expr | const\}]} dove
  \begin{itemize}[label=$\circ$, itemsep=\smallskipamount]
    \item \texttt{expr := espressione} che contiene riferimenti agli attributi delle tabelle che compaiono nella clausola \texttt{FROM}
    \item $\theta \in$ \texttt{\{=,<>,<,>,<=,>=\}}
    \item \texttt{const := valore} dei domini di base
  \end{itemize}
  \item una riga soddisfa \texttt{condition} se l’espressione è vera quando valutata con i valori della riga
...
\subsubsection{Operatore \texttt{LIKE}}
\begin{codiceSQL}
WHERE attributo [NOT] LIKE 'pattern';
\end{codiceSQL}

Nella clausola \texttt{WHERE} può apparire l’operatore \texttt{LIKE} per il confronto di stringhe.\\[0.2ex]
\texttt{LIKE} è un operatore di pattern matching.

\smallskip
I pattern si costruiscono con i caratteri speciali:
\begin{itemize}[label=$\triangleright$, itemsep=\smallskipamount]
...
\subsubsection{Operatore \texttt{SIMILAR TO}}
L’operatore \texttt{SIMILAR TO} è un \texttt{LIKE} più espressivo che accetta, come pattern,  un sottoinsieme delle espressioni regolari POSIX (versione SQL).
...
\subsubsection{Operatore \texttt{BETWEEN}}

\begin{codiceSQL}
WHERE attributo [NOT] BETWEEN valore_iniziale AND valore_finale;
\end{codiceSQL}

Nella clausola \texttt{WHERE} può apparire l’operatore \textcolor{purple}{\textbf{BETWEEN}} per testare l’appartenenza di un valore a un intervallo (non limitato ai valori numerici, usato anche con date e stringhe).
...
\subsubsection{Operatore \texttt{IN}}
\begin{codiceSQL}
  WHERE attributo [NOT] IN ( valore [, ...] );

\end{codiceSQL}
Nella clausola \texttt{WHERE} può apparire l’operatore \texttt{[NOT] IN} per testare  l’appartenenza di un valore ad un insieme.
\begin{codiceSQL}
SELECT *
FROM Persone
...
\subsubsection{Operatore \texttt{IS NULL}}
\begin{codiceSQL}
WHERE attributo IS [ NOT ] NULL;
\end{codiceSQL}

Nella clausola \texttt{WHERE} può apparire l’operatore \texttt{IS [NOT] NULL} per testare se un valore è \texttt{NOT KNOWN (NULL)} o no.\\[0.2ex]
Il valore \texttt{NULL} non rappresenta lo zero o una stringa vuota, ma l'assenza di dati 
o un valore sconosciuto.\\[0.2ex]
A partire dalle versioni SQL-2 è stata adottata una logica a tre valori (\texttt{true},  \texttt{false}, \texttt{unknown}) e i valori nulli non sono considerati né veri, né falsi.
\begin{center}
\textbf{NON SI PUÒ usare ’=’ o ’<>’ con il valore NULL! } 
\end{center}
...
\subsubsection{Operatore \texttt{ORDER BY}}
\begin{codiceSQL}
ORDER BY attributo [{ ASC | DESC }] [, ... ];
\end{codiceSQL}

La clausola \texttt{ORDER BY} ordina le tuple del risultato in ordine rispetto agli attributi  specificati (ASCendente è lo standard).
\begin{codiceSQL}
SELECT Nome
FROM Persone
ORDER BY Nome DESC;
...
Sono operatori che permettono di determinare un valore considerando i valori ottenuti da una \texttt{SELECT}.

\smallskip
Due tipi principali:
\begin{itemize}[label=$\triangleright$, itemsep=\smallskipamount]
  \item \texttt{COUNT}
  \item \texttt{MAX, MIN, AVG, SUM}
\end{itemize}
...
\vario{Nota}{
  Quando si usano gli operatori aggregati, dopo la \texttt{SELECT} non possono comparire espressioni che usano i valori presenti nelle singole tuple perché il risultato è sempre e solo una tupla.
}
...
\end{codiceSQL}
  ...
  Tre casi comuni:\begin{itemize}[label=$\diamond$, itemsep=\smallskipamount]
    \item \texttt{COUNT(*)}: ritorna il numero di tuple nel risultato dell’interrogazione
    \item \texttt{COUNT(expr)}: ritorna il numero di tuple in ciascuna delle quali il valore expr è non nullo
    \item \texttt{COUNT(ALL expr)}: include duplicati e valori nulli
    \item \texttt{COUNT(DISTINCT expr)} come con \texttt{COUNT(expr)} ma con l’ulteriore condizione che i valori di expr siano distinti
  \end{itemize}
  \item \texttt{SUM/MAX/MIN/AVG} 
\begin{codiceSQL}
...
\end{codiceSQL}
  ...
  \begin{itemize}[label=$\diamond$, itemsep=\smallskipamount]
    \item \texttt{op = SUM | AVG | MAX | MIN}
    \item \texttt{expr}: espressione che usa uno o più attributi e funzioni di attributi
    \item \texttt{DISTINCT} considero solo i valori distinti
  \end{itemize}
\begin{codiceSQL}
...
\subsection{Interrogazione con raggruppamento: \texttt{GROUP BY}}
Un raggruppamento è un insieme di tuple che hanno medesimi valori su uno o più attributi caratteristici del raggruppamento.\\[0.2ex]
La clausola \texttt{GROUP BY attr [, ...]} permette di determinare tutti i raggruppamenti delle tuple della relazione risultato (tuple selezionate con la clausola \texttt{WHERE}) in funzione degli attributi dati.\\[0.2ex]
In una interrogazione che usa GROUP BY, la clausola \texttt{SELECT} contiene solamente gli attributi (da 0 a tutti) utilizzati per il raggruppamento ed eventuali funzioni di aggregazione sugli altri attributi.

\begin{esempiobox}{Visualizzare tutte le città raggruppate della tabella \texttt{Studente}}
\begin{codiceSQL}
SELECT citta
FROM studente
GROUP BY citta
...
\begin{esempiobox}{Visualizzare tutte le città e i cognomi raggruppati}
...
\vario{Nota}{
  Nel \texttt{GROUP BY} non si possono usare espressioni con operatori di aggregazione.
}

\vario{Nota}{
  Nella \texttt{SELECT} con \texttt{GROUP BY} non si possono specificare attributi che non sono raggruppati.
}

\begin{itemize}[label=$\circ$, itemsep=\smallskipamount]
  \item la clausola \texttt{WHERE} permette di selezionare le righe che devono far parte del risultato
  \item la clausola \texttt{HAVING} permette di selezionare i raggruppamenti che devono far parte del risultato
  \item la sintassi è \texttt{HAVING bool\_expr}, dove \texttt{bool\_expr} è un’espressione booleana che può usare gli attributi usati nel \texttt{GROUP BY} e/o gli altri attributi mediante operatori di aggregazione
\end{itemize}

\begin{esempiobox}{Visualizzare tutte le città raggruppate che iniziano con ’V’ della tabella \texttt{Studente}}
  \begin{minipage}{0.5\textwidth}
\begin{codiceSQL}
SELECT citta
FROM Studente
...
\begin{esempiobox}{Visualizzare tutte le città e i cognomi raggruppati con almeno due studenti con lo stesso cognome..}
  \begin{minipage}{0.5\textwidth}
   \begin{codiceSQL}
SELECT cognome, LOWER(citta)
FROM Studente
...
\begin{esempiobox}{Si possono specificare espressioni con operatori di aggregazione su attributi non raggruppati anche nella clausola \texttt{HAVING}}
  \begin{minipage}{0.5\textwidth}
\begin{codiceSQL}
...
\subsection{Interrogazione con \texttt{JOIN}}
